...
Sentry._init=Sentry.init,Sentry.init=function(a){var b,c,d,e,f,g,h,i,j;if(!a.dsn)return Sentry._init(a),void 0;if(b=a.dsn.replace(/(https:)|(http:)/,document.location.protocol).replace(/@.*com?/,"@"+document.domain+"/apmfe"),a.dsn=b,c=window._okGlobal||window.okGlobal,d=c?c.envSign:"",d&&(a.environment=d),e=c?c.cdnBaseUrl:"",e&&"string"==typeof e&&(a.allowUrls=[window.origin,e]),f=RegExp(/daily/),!d.match(f)){Sentry._init(a);try{g=localStorage.getItem("ok_global"),h=g?JSON.parse(g):{},i=h.profile||{},j=i.uuid,j&&Sentry.setUser({id:j})}catch(k){}}};
